...
public class ModalListener extends ListenerAdapter {
    ...
    public void onModalInteraction(@NotNull ModalInteractionEvent event) {
        ...
        } else if (event.getModalId().startsWith("modRename")) {
            String newName = event.getValue("newName").getAsString();
            String id = event.getModalId().substring(10);
            Member target = event.getGuild().getMemberById(id);

            target.modifyNickname(newName).queue();

            event.reply("renamed "+target.getUser().getName()+"'s name to "+newName).setEphemeral(true).queue();
        }
    }
